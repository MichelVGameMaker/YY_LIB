...
Now, each step that you'd like to shw some ImGui, just call all of the functions that make up your UI. See the example provided for details!
...
```
get gml color from an imgui color
...
```
set the imguigml display mode - this affects how the window is resized / handled.
...
**returns**: {Real:EImGui_DisplayMode} ImGuIGML'
```
imguigml_get_display_mode()
```
...
```
set the imguigml display scaling - it will scale the mouse and the vertex buffer ImGui passes ImGuiGML by this much.
...
```
set the display size (how imgui will clamp windows).
...
**returns** | None
```
imguigml_drawlist_add_line(_x1, _y1, _x2, _y1, _color, [_thickness=1.0])
```
...
**_col**                       | {Real} 
**[_rounding=0.0]**            | {Real} 
**[_rounding_corner_flags=0]** | {Real:EImGui_DrawFlags} 
**[_thickness=1.0]**           | {Real} 
**returns** | None
```
imguigml_drawlist_add_rect(_x1, _y1, _x2, _y2, _col, [_rounding = 0.0], [_rounding_corner_flags = ~0], [_thickness = 1.0])
```
...
**returns** | None
```
imguigml_drawlist_add_rect_filled(_x1, _y1, _x2, _y2, _col, [_rounding = 0.0], [_rounding_corner_flags = ~0])
```
...
**returns** | None
```
imguigml_drawlist_add_quad(_x1, _y1, _x2, _y2, _x3, _y3, _x4, _y4, _col, [_thickness = 1.0])
```
...
**returns** | None
```
imguigml_drawlist_add_quad_filled(_x1, _y1, _x2, _y2, _x3, _y3, _x4, _y4, _col, [_thickness = 1.0])
```
...
**returns** | None
```
imguigml_drawlist_add_circle(_center_x, _center_y, _radius, _col, [_num_segments=12], [_thickness=1.0])
```
...
**returns** | None
```
imguigml_drawlist_add_surface(_surface, _x, _y, [_width=undefined], [_height=undefined], [_u0=0.0], [_v0=0.0], [_u1=1.0], [_v1=1.0], [color=c_white])
```
...
**returns** | None
```
imguigml_drawlist_add_surface_quad(surface, _x1, _y1, _x2, _y2, x3, _y3, _x4, _y4, [_u1=0], [_v1=0], [_u2=1], [_v2=0], [_u3=1], [_v3=1], [_u4=0], [_v4=1], [_col=c_white])
```
...
**returns** | None
```
imguigml_drawlist_add_sprite(_sprite_index, _sub_img, _x, _y, [_width=undefined], [_height=undefined], [_col=c_white])
```
...
**returns** | None
```
imguigml_drawlist_add_polyline(_points, _col, [_closed=true], [_thickness=1.0], [_anti_aliased=true])
```
...
**_max_y**                  | {Real} 
**[_rounding=0]**           | {Real} 
**[_round_corner_flags=0]** | {Real:EImGui_DrawFlags} 
**returns** | None
```
imguigml_drawlist_path_rect(_x1, _x2, _y1, _y2, [_rounding=0.0], [_rounding_corner_flags=-1])
```
...
**returns**: {Real:EImGui_DrawListFlags} 
```
imguigml_drawlist_get_flags()
```
...
**returns** | None
```
__imguigml_drawlist_add_image_quad(_texture_id, _x1, _y1, _x2, _y2, x3, _y3, _x4, _y4, [_u1=0], [_v1=0], [_u2=1], [_v2=0], [_u3=1], [_v3=1], [_u4=0], [_v4=1], [_col=c_white])
```
Adds an image quad to the draw list
...
**current** | returns { Real } time to use
**returns** | None
```
...
**current**     | returns { Real } max distance
**returns** | None
```
...
**current**     | returns { Real } distance threshold
**returns** | None
```
...
**current** | returns { Real } repeat rate
**returns** | None
```
...
**current**  | returns { Real } font scale
**returns** | None
```
...
```
OS X style: Text editing cursor movement using Alt instead of Ctrl, Shortcuts using Cmd/Super instead of Ctrl, Line/Text Start
...
**returns** | {String} current ini f
```
imguigml_io_ini_filename([_path])
```
...
**returns** | {String} current log filena
```
...
| | | 
| ----- | ----------------------------- |
**the** | filename {String} path to use
**returns** | None
```
imguigml_secure_load_mem(filename)
```
...
**returns** | None
```
imguigml_secure_save_mem(filename)
```
...
**returns** | None
```
imguigml_dummy(_size_x, _size_y)
```
...
**returns**: {Real} height of font == GetWindowFontS
```
imguigml_get_text_line_height()
```
...
**returns**: {Real} distance (in pixels) between 2 consecutive lines of text == GetWindowFontSize() + GetStyle().ItemSpac
```
imguigml_get_text_line_height_with_spacing()
```
...
**returns**: {Real} distance (in pixels) between 2 consecutive lines of standard height widgets == GetWindowFontSize() + GetStyle().FramePadding.y*2 + GetStyle().ItemSpac
```
imguigml_get_frame_height_with_spacing()
```
...
**returns** | None
```
imguigml_begin_drag_drop_source([_flags=0])
```
...
**returns** | {Bool} to be quite frank, i'm not sure what it
```
...
**_type**      | {String} type of payload. limited to 8 characters, starting with "_" is reserved for ImGui
**[_flags=0]** | {Real:EImGui_Cond} 
**returns** | {Array|undefined} undefined if no drop, else [ _payload, _is_preview, _is_delivery ] delivery is when we've released the mouse button, preview we a
```
...
**returns** | {Real} _id to pass to dragdrop func
```
imguigml_payload([_id], _payload_data)
```
if an id is passed, use this id when looking up the payload, otherwise, return a generated id.
...
```
imguigml_add_font_from_ttf(_file_path, [_font_size=10])
```
Add font from a TTF file, passing the filepath to it. Returns a font index that can be checked for validity with `imguigml_is_font_valid`
...
**returns**: None
```
imguigml_push_font(_font_index)
```
...
imguigml_get_font()
```
get current font (not implemented)

---

#### `imguigml_get_font_size`
**returns**: {Real} current font
```
imguigml_get_font_size()
```
...
#### `imguigml_is_font_valid`
**returns**: {Bool} is the font
```
imguigml_is_font_valid(_font_index)
```
returns true if the font given is a valid font
...
**returns**: None
```
imguigml_set_clipboard_text(_text)
```
...
**_repeat_delay** | {Real} 
**_rate**         | {Real} 
**returns** | {Real} return a count, most often 0 or 1 but might be >1 if RepeatRate is small enough that DeltaTime > Repea
```
...
**returns** | {Boolean} 
```
imguigml_is_mouse_dragging([_button=0], [_lock_threadhold=-1.0])
```
is mouse dragging.
...
**returns**: {Real:EImGui_MouseCursor} 
```
imguigml_get_mouse_cursor()
```
...
| --------- | --------- |
**_str_id** | {String} 
**returns** | {Boolean} true if the popup is open, and you can start outputtin
```
...
**returns** | {Boolean|Array:[Boolean, Boolean]} if `_open` is passed, then an arr
```
...
**returns** | {Array:[{Boolean} _changed, {Real} _
```
...
```
imguigml_begin_tab_item(_name, [_open=undefined], [_flags=0])
```
create a tab item
...
| --------------- | ------- |
**_editor_index** | {Real} 
**returns** | {undefined} so you can c
```
imguigml_texteditor_destroy(_editor)
```
...
**returns** | None
```
imguigml_texteditor_set_language_definition(_editor_index)
```
...
| --------------- | ------- |
**_editor_index** | {Real} 
**returns** | {Array:EImGuiText_Palette} filled in array wi
```
...
**returns** | None
```
imguigml_texteditor_move_up(_editor_index, _amount, [_select = false])
```
...
**returns** | None
```
imguigml_texteditor_move_down(_editor_index, _amount, [_select = false])
```
...
**returns** | None
```
imguigml_texteditor_move_left(_editor_index, _amount, [_select = false])
```
...
**returns** | None
```
imguigml_texteditor_move_right(_editor_index, _amount, [_select = false])
```
...
**returns** | None
```
imguigml_texteditor_move_top(_editor_index, [_select = false])
```
...
**returns** | None
```
imguigml_texteditor_move_bottom(_editor_index, [_select = false])
```
...
**returns** | None
```
imguigml_texteditor_move_home(_editor_index, [_select = false])
```
...
**returns** | None
```
imguigml_texteditor_move_end(_editor_index, [_select = false])
```
...
**returns** | None
```
imguigml_texteditor_set_selection(_editor_index, _line1, _col1, _line2, _col2, [word_mode = false])
```
...
**returns** | None
```
imguigml_value(_label, _val_type, _val, [_float_format])
```
...
**returns** | {Array:[r,g,b]} 
```
imguigml_color_convert_hsv_to_rgb(_h, _s, _v)
```
convert rgb to hsv
...
**returns** | {Boolean} 
```
imguigml_is_rect_visible(_sizeOrMin, [_max])
```
...
**returns**: {Boolean} is current window c
```
imguigml_is_window_collapsed()
```
...
**returns**: {Boolean} is current window a
```
imguigml_is_window_appearing()
```
...
**returns** | None
```
imguigml_set_next_window_pos(_pos_x, _pos_y, [_cond=0], [_pivot_x=0], [_pivot_y=0])
```
...
**returns** | None
```
imguigml_set_next_window_size(_size_x, _size_y, [_cond=0])
```
...
**returns** | None
```
imguigml_set_next_window_size_constraints(_min_size_x, _min_size_y, _max_size_x, _max_size_y)
```
...
**returns**: {Real} scrolling amount ~~ ContentSize.X - WindowS
```
imguigml_get_scroll_max_x()
```
...
**returns**: {Real} scrolling amount ~~ ContentSize.Y - WindowS
```
imguigml_get_scroll_max_y()
```
...
**returns** | None
```
imguigml_set_scroll_from_pos_y(_pos_y, [_center_y_ratio=0.5])
```
...
**returns** | None
```
imguigml_set_keyboard_focus_here([_offset=0])
```
...
```
shw a font selector **NOTE:** Functionality is limited at this time until `ImGuiGML` has better font support
...
**returns** | {Boolean} 
```
imguigml_button(_label, [_size_x=0], [_size_y=0])
```
ImGui::Button()
...
**returns** | {Boolean} 
```
imguigml_invisible_button(_str_id, [_size_x=0], [_size_y=0])
```
...
**returns** | None
```
imguigml_sprite(_sprite_index, _sub_img, [_width=undefined], [_height=undefiend], [_tint_r=1.0], [_tint_g=1.0], [_tint_b=1.0], [_tint_a=1.0], [_border_r=0.0], [_border_g=0.0], [_border_b=0.0], [_border_a=0.0])
```
ImGui::Image()
...
**returns** | {Boolean} 
```
imguigml_image_button(_texture_id, _size_x, _size_y, [_u0=0.0], [_v0=0.0], [_u1=1.0], [_v1=1.0], [_bg_r=0.0], [_bg_g=0.0], [_bg_b=0.0], [_bg_a=0.0], [_tint_r=1.0], [_tint_g=1.0], [_tint_b=1.0], [_tint_a=1.0])
```
ImGui::ImageButton()
...
**returns** | {Boolean} 
```
imguigml_surface_button(_surface, _size_x, _size_y, [_u0=0.0], [_v0=0.0], [_u1=1.0], [_v1=1.0], [_bg_r=0.0], [_bg_g=0.0], [_bg_b=0.0], [_bg_a=0.0], [_tint_r=1.0], [_tint_g=1.0], [_tint_b=1.0], [_tint_a=1.0])
```
...
**returns** | {Boolean} 
```
imguigml_sprite_button(_sprite_index, _sub_img, [_width=undefined], [_height=undefiend], [_bg_r=0.0], [_bg_g=0.0], [_bg_b=0.0], [_bg_a=0.0], [_tint_r=1.0], [_tint_g=1.0], [_tint_b=1.0], [_tint_a=1.0])
```
ImGui::ImageButton()
...
| ---------------- | ------------------------------------------------------------------------------------------------------ |
**_label**         | {String} button label
**_activeOrVal**   | {Real|Boolean} pass true/false or the index of the active radio button in a group
**[_current_val]** | {Real} current value of radio buttons, if passed, will return the index of the active radio in a group
**returns** | {Boolean|[Boolean, Real]} pressed, or if a value is passed, the va
```
imguigml_radio_button(_label, _active, [_current_val])
```
...
**returns** | None
```
imguigml_progress_bar(_fraction, [_size_x=-1.0], [_size_y=0.0], [_overlay])
```
...
**returns** | {Boolean} 
```
__imguigml_image_button(_texture_id, _size_x, _size_y, [_u0=0.0], [_v0=0.0], [_u1=1.0], [_v1=1.0], [_tint_r=1.0], [_tint_g=1.0], [_tint_b=1.0], [_tint_a=1.0], [_border_r=0.0], [_border_g=0.0], [_border_b=0.0], [_border_a=0.0])
```
...
```
imguigml_combo(_label, _current_item, _items, [_height_in_items = -1]);
```
...
**returns** | {Bool} 
```
imguigml_begin_combo(_label, _preview_value, [_flags=0])
```
...
#### `imguigml_set_item_default_focus`
**returns**: None
```
imguigml_set_item_default_focus()
```
set the default item focus (temp function until nav update)
...
**[_flags=EImGui_SliderFlags.None]** | {Real:EImGui_SliderFlags} 
**returns** | {Array:[_changed, _v]} 
```
imguigml_drag_float(_label, _v, [_v_speed=1.0], [_v_min=0.0], [_v_max=0.0], [_display_format="%.3f"], [_flags=EImGui_SliderFlags.None]);
```
...
```
imguigml_drag_float2(_label, _v, _v2, [_v_speed=1.0], [_v_min=0.0], [_v_max=0.0], [_display_format="%.3f"], [_flags=EImGui_SliderFlags.None]);
```
If v_min >= v_max we have no bound
...
```
imguigml_drag_float3(_label, _v, _v2, _v3, [_v_speed=1.0], [_v_min=0.0], [_v_max=0.0], [_display_format="%.3f"], [_flags=EImGui_SliderFlags.None]);
```
If v_min >= v_max we have no bound
...
```
imguigml_drag_float4(_label, _v, _v2, _v3, _v4, [_v_speed=1.0], [_v_min=0.0], [_v_max=0.0], [_display_format="%.3f"], [_flags=EImGui_SliderFlags.None]);
```
If v_min >= v_max we have no bound
...
```
imguigml_drag_float_range2(_label, _v_current_min, _v_current_max, [_v_speed=1.0], [_v_min=0], [_v_max=0], [_display_fmt="%.0f"], [_display_fmt_max=undefined], [_flags=EImGui_SliderFlags.None]);
```
...
**returns** | {Array:[_changed, _v]} 
```
imguigml_drag_int(_label, _v, [_v_speed=1.0], [_v_min=0.0], [_v_max=0.0], [_display_format="%.3f"]);
```
If v_min >= v_max we have no bound
...
**returns** | {Array:[_changed, _v, _v2]} 
```
imguigml_drag_int2(_label, _v, _v2, [_v_speed=1.0], [_v_min=0.0], [_v_max=0.0], [_display_format="%.3f"]);
```
If v_min >= v_max we have no bound
...
**returns** | {Array:[_changed, _v, _v2, _v3]} 
```
imguigml_drag_int3(_label, _v, _v2, _v3, [_v_speed=1.0], [_v_min=0.0], [_v_max=0.0], [_display_format="%.3f"]);
```
If v_min >= v_max we have no bound
...
**returns** | {Array:[_changed, _v, _v2, _v3, _v4]} 
```
imguigml_drag_int4(_label, _v, _v2, _v3, _v4, [_v_speed=1.0], [_v_min=0.0], [_v_max=0.0], [_display_format="%.3f"]);
```
If v_min >= v_max we have no bound
...
**returns** | {Array:[_changed, _min, _max]} 
```
imguigml_drag_int_range2(_label, _v_current_min, _v_current_max, [_v_speed=1.0], [_v_min=0], [_v_max=0], [_display_fmt="%.0f"], [_display_fmt_max=undefined])
```
drag an integer range 2?
...
| | | 
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
**_label**                | {String} label to shw next to the input
...
```
imguigml_input_text(_label, _text, _max_length, [_flags=0], [_callback=undefined], [_user_data=undefined])
```
...
**returns** | {Array:[_changed, _text]} 
```
imguigml_input_text_multiline(_label, _text, _max_len, _size_x, _size_y, [_flags=0], [_callback=undefined], [_user_data=undefined])
```
add an input text field
...
**returns** | {Array:[_changed, _val]} 
```
imguigml_slider_float(_label, _val, _min, _max, [_display_fmt="%.3f"], [_flags=EImGui_SliderFlags.None]);
```
...
**returns** | {Array:[_changed, _val]} 
```
imguigml_slider_float2(_label, _val, _val2, _min, _max, [_display_fmt="%.3f"], [_flags=EImGui_SliderFlags.None]);
```
adjust display_format to decorate the value with a prefix or a suffix for in-slider labels or unit display.
...
**returns** | {Array:[_changed, _val]} 
```
imguigml_slider_float3(_label, _val, _val2, _val3, _min, _max, [_display_fmt="%.3f"], [_flags=EImGui_SliderFlags.None])
```
adjust display_format to decorate the value with a prefix or a suffix for in-slider labels or unit display.
...
**returns** | {Array:[_changed, _val]} 
```
imguigml_slider_float4(_label, _val, _val2, _val3, _val4, _min, _max, [_display_fmt="%.3f"], [_flags=EImGui_SliderFlags.None])
```
adjust display_format to decorate the value with a prefix or a suffix for in-slider labels or unit display.
...
**returns** | {Array:[_changed, _v]} 
```
imguigml_vslider_float(_label, _size_x, _size_y,  _v, _v_min, _v_max, [_display_fmt="%.0f"], [_flags=EImGui_SliderFlags.None])
```
a vslider float?
...
**returns** | {Boolean} if returning 'true' the node is open and the tree id is pushed into the id stack. user is responsible for calling Tr
```
...
**[_flags=0]**        | {Real:EImGui_TreeNodeFlags]} 
**returns** | {Array:[_collapsed, [_open]} if returning 'true' the header is open. doesn't indent nor push on ID stack. user does
```
imguigml_collapsing_header(_label, [_p_open=undefined], [_flags=0])
```
